...
initScene = function() 
{
	...
	new THREE.MeshLambertMaterial({map: new THREE.TextureLoader().load('img/graham.jpg')}), pf, pr  
	  );
	...
	car.frame.add( goal );        
	goal.position.set(15, 3, 0); //target position for the camera
...
function render() {
	...
	camera.position.lerp(curTarget, 0.8);				//won't do anything if (vector3) camera.position == curTarget
	camera.lookAt(car.frame.position);					//angle the camera back at the trolley if new curTarget
	camera.position.y += 4; 							//manually move the camera up to get a wider view
